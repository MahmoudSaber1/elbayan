...
export const UpdateStudentForm = ({ onCancel, initialValues }: UpdateStudentFormProps<Student>) => {
    ...
    const onSubmit = (values: z.infer<typeof createStudentSchema>) => {
        const finalValues = { ...values, profilePicture: values.profilePicture instanceof File ? values.profilePicture : "", nationalPicture: values.nationalPicture instanceof File ? values.nationalPicture : "" };
        updateStudentMutate(
            { form: finalValues as any, param: { studentId: initialValues.$id } },
            {
                onSuccess: () => {
                    form.reset();
                    onCancel?.();
